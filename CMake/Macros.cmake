...
macro(gexl_static_add_libraries target)
...
macro(gexl_list_contains var value)
  set(${var})
  foreach(value2 ${ARGN})
...
# ex: gexl_parse_arguments(THIS "SOURCES;DEPENDS" "FLAG" FLAG SOURCES s1 s2 s3 DEPENDS d1 d2)
# will define the following variables:
# - THIS_SOURCES (s1 s2 s3)
# - THIS_DEPENDS (d1 d2)
# - THIS_FLAG TRUE
macro(gexl_parse_arguments prefix arg_names option_names)
  ...
  foreach(arg ${ARGN})
    gexl_list_contains(is_arg_name ${arg} ${arg_names})
    if(is_arg_name)
      set(${prefix}_${current_arg_name} ${current_arg_list})
      set(current_arg_name ${arg})
      set(current_arg_list)
    else()
      gexl_list_contains(is_option ${arg} ${option_names})
      if(is_option)
...
# add a new target which is a GExL library
# ex: gexl_add_library(gexl-core
#                     HEADER        include/GExL/Core.hpp
#                     HEADER_DIR    include/GExL/Core
#                     SOURCES       include/GExL/Core/classes/App.hpp src/GExL/Core/classes/App.cpp ...
#                     DEPENDS       ...
#                     EXTERNAL_LIBS sfml-audio sfml-graphics sfml-window sfml-system ...)
macro(gexl_add_library target)
  # parse the arguments
  gexl_parse_arguments(THIS "HEADER;HEADER_DIR;INCLUDES;SOURCES;DEPENDS;EXTERNAL_LIBS" "" ${ARGN})
  ...
  if(BUILD_SHARED_LIBS)
    if(WINDOWS)
      ...
      set_target_properties(${target} PROPERTIES SUFFIX "-${GExL_VERSION_MAJOR}_${GExL_VERSION_MINOR}${CMAKE_SHARED_LIBRARY_SUFFIX}")
    else()
      set_target_properties(${target} PROPERTIES DEBUG_POSTFIX -d)
    endif()
    if (WINDOWS AND COMPILER_GCC)
  ...
  set_target_properties(${target} PROPERTIES SOVERSION ${GExL_VERSION_MAJOR}.${GExL_VERSION_MINOR})
  set_target_properties(${target} PROPERTIES VERSION ${GExL_VERSION_MAJOR}.${GExL_VERSION_MINOR}.${GExL_VERSION_PATCH})
  ...
  # link the target to its GExL dependencies
  if(THIS_DEPENDS)
  ...
  if(THIS_EXTERNAL_LIBS)
    if(BUILD_SHARED_LIBS)
      ...
      gexl_static_add_libraries(${target} ${THIS_EXTERNAL_LIBS})
...
# add a new target which is a GExL example
# ex: gexl_add_example(demo
#                     SOURCES main.cpp ...
#                     DEPENDS gexl-core gexl-standalone)
macro(gexl_add_example target)

    # parse the arguments
    gexl_parse_arguments(THIS "SOURCES;DEPENDS" "GUI_APP" ${ARGN})
